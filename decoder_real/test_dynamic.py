...
# from demodulate_baseband import (
#     costas_loop_enhanced,
#     find_optimal_loop_bandwidth,
#     evaluate_clustering_quality,
#     create_rrc_filter,
#     match_filter_and_sample,
#     demodulate
# )
...
# 优先使用split_from_raw_data.py中的评估函数，确保一致性
# try:
#     from split_from_raw_data import demodulate_signal_consistent
#     USE_CONSISTENT_DEMOD = True
# except ImportError:
#     USE_CONSISTENT_DEMOD = False
#     print("警告: 无法导入split_from_raw_data中的评估函数，使用本地实现")
USE_CONSISTENT_DEMOD = False
...
def process_one_item(params_list):
    n, data1, data2, data3, data4, args, output_dir = params_list
    optimal_snr1 = -100
    optimal_snr2 = -100
    optimal_ser1 = 1.0
    optimal_ser2 = 1.0
    ...
    for data in [data4]:
        ...
        cur_cluster_score = result1['pred_clustering_score'] + result2['pred_clustering_score']
        if cur_cluster_score > best_cluster_score:
            best_cluster_score = cur_cluster_score
            optimal_snr1 = snr1
            optimal_snr2 = snr2
            optimal_ser1 = result1['ser']
            optimal_ser2 = result2['ser']
            optimal_score1 = result1['pred_clustering_score']
            optimal_score2 = result2['pred_clustering_score']
        # ------ 保存结果文件（每个进程写自己文件） ------
    ...
    return {
        ...
        "ber1": optimal_ser1,
        "ber2": optimal_ser2,
        "score1": optimal_score1,
        "score2": optimal_score2,
        "n": n
    }
...
def run_all(data, args, output_dir, num_workers=8):
    ...
    # index_list = list(range(0, len(data1), 10))
    index_list = list(range(7500, 10000, 10))
    ...
    score_signal_1 = []
    score_signal_2 = []
    params_list = [(n, data1[n], data2[n], data3[n], data4[n], args, output_dir) for n in index_list]
    ...
    with mp.Pool(num_workers) as pool:
        ...
        for res in pool.imap(process_one_item, params_list):
            ...
            score_signal_1.append(res["score1"])
            score_signal_2.append(res["score2"])

    # 最终返回或保存（你自行决定）
    return snr_signal_1, snr_signal_2, ber_signal_1, ber_signal_2, score_signal_1, score_signal_2
...
#     如果可用，优先使用split_from_raw_data.py中的demodulate_signal_consistent函数，
#     确保与测试集评估时的方法完全一致
...
#         - info_dict: 包含解调信息的字典（loop_bandwidth, offset等）
#     """
#     # 如果可用，优先使用split_from_raw_data.py中的函数
#     # 确保与test_ser_with_same_signal.py使用相同的方式
#     if USE_CONSISTENT_DEMOD:
#         # 始终使用split_from_raw_data.py中的函数，确保一致性
#         return demodulate_signal_consistent(
#             signal, modulation, 
#             loop_bandwidth=loop_bandwidth,
#             auto_search=auto_search,
#             threshold=6.0
...
#     # 自动搜索最优loop_bandwidth
#     # 注意：使用与split_from_raw_data.py相同的参数，确保一致性
#     if loop_bandwidth is None and auto_search:
#         loop_bandwidth, score_temp, _, signal_comp, offset = find_optimal_loop_bandwidth(
#             signal, sps=SPS, fs=FS, modulation=modulation,
#             target_score=6.0,  # 与split_from_raw_data.py一致
#             min_bw=1e-14,      # 与split_from_raw_data.py一致
#             max_bw=1e-5,       # 与split_from_raw_data.py一致
#             num_trials=50,     # 与split_from_raw_data.py一致
#             verbose=False
#         )
#         # signal_comp是补偿后的信号，需要进行匹配滤波
#         rrc_filter = create_rrc_filter(BETA, SPS, NUM_TAPS)
#         signal_mf, _ = match_filter_and_sample(signal_comp, rrc_filter, SPS, NUM_TAPS)
#         # 匹配滤波后，重新找最佳offset（与split_from_raw_data.py一致）
#         # 注意：find_optimal_loop_bandwidth返回的offset是在匹配滤波前找到的，
#         # 但实际使用时需要在匹配滤波后重新搜索，因为匹配滤波会改变信号时序
#         best_offset = 0
#         best_score_mf = -np.inf
#         for offset_mf in range(SPS):
#             symbols_temp = signal_mf[offset_mf::SPS]
#             if len(symbols_temp) > 0:
#                 score_mf = evaluate_clustering_quality(symbols_temp, modulation)
#                 if score_mf > best_score_mf:
#                     best_score_mf = score_mf
#                     best_offset = offset_mf
#         signal_compensated = signal_mf
#         freq_offset_est = None  # find_optimal_loop_bandwidth不直接返回频偏估计
#         initial_phase = None
#     else:
...
#         # 匹配滤波
#         rrc_filter = create_rrc_filter(BETA, SPS, NUM_TAPS)
#         signal_mf, _ = match_filter_and_sample(signal_compensated, rrc_filter, SPS, NUM_TAPS)
...
#                     best_score = score
#                     best_offset = offset
#                     best_symbols = symbols_temp
        
#         signal_compensated = signal_mf
#         score_temp = best_score
    
#     # 使用最佳offset抽取符号
#     symbols = signal_compensated[best_offset::SPS]
...
#         'loop_bandwidth': loop_bandwidth,
#         'offset': best_offset,
#         'freq_offset_est': freq_offset_est if 'freq_offset_est' in locals() else None,
#         'initial_phase': initial_phase if 'initial_phase' in locals() else None,
...
def main():
    ...
    parser.add_argument("--pred_file", type=str, default=None,
                       help="模型输出信号文件（.pth格式，单个切片，或包含pred和gt两个信号的文件，可选）")
    parser.add_argument("--disable_middle_output", action="store_true",
                       help="模型输出信号文件（.pth格式，单个切片，或包含pred和gt两个信号的文件，可选）")
    parser.add_argument("--gt_file", type=str, default=None,
                       help="Ground truth文件（test的pth文件，包含多个切片。如果pred_file包含gt，则可不提供）")
    parser.add_argument("--modulation", type=str, required=True,
                       choices=["QPSK", "8PSK", "16QAM"],
                       help="调制方式")
    parser.add_argument("--signal_idx", type=int, default=0,
                       choices=[0, 1],
                       help="信号索引（0=rfsignal1, 1=rfsignal2）")
    parser.add_argument("--entry_idx", type=int, default=0,
                       help="pth文件中的切片索引（第几个切片，默认0）")
    ...
    # # 如果提供了pred_file，尝试从中加载两个信号
    # if not args.gt_only and args.pred_file:
    #     print(f"\n加载模型输出和ground truth（从单个文件）...")
    #     print(f"  文件: {args.pred_file}")
    #     try:
    #         pred_signal, gt_signal = load_two_signals_from_npy(args.pred_file)
    #         print(f"  Pred信号长度: {len(pred_signal)} 采样点")
    #         print(f"  GT信号长度: {len(gt_signal)} 采样点")
    #     except Exception as e:
    #         print(f"  ✗ 从pred_file加载两个信号失败: {e}")
    #         print(f"  尝试只加载pred信号...")
    #         try:
    #             pred_signal = load_signal_from_npy(args.pred_file)
    #             print(f"  Pred信号长度: {len(pred_signal)} 采样点")
    #         except Exception as e2:
    #             print(f"  ✗ 加载pred信号也失败: {e2}")
    #             pred_signal = None
    print('loading data from {}'.format(args.pred_file))
    data1 = torch.load('/nas/datasets/zjw/PCMA_8psk/real/dynamic/train_0.6/shard1.pth', weights_only=False)
    data2 = torch.load('/nas/datasets/zjw/PCMA_8psk/real/dynamic/train_0.7/shard1.pth', weights_only=False)
    data3 = torch.load('/nas/datasets/zjw/PCMA_8psk/real/dynamic/train_0.8/shard1.pth', weights_only=False)
    data4 = torch.load('/nas/datasets/zjw/PCMA_8psk/real/dynamic/train_0.9/shard1.pth', weights_only=False)
    print('data loaded.')
    ber_signal_1 = []
    ber_signal_2 = []
    snr_signal_1 = []
    snr_signal_2 = []
    # for n in tqdm.tqdm(range(0, len(data), 100)):
    #     pred_signal, gt_signal = load_two_signals_from_npy(data[n])
    #     # print(pred_signal)
    #     # print(gt_signal)
    #     # 如果gt_signal还没有加载，从gt_file加载
    #     if gt_signal is None:
    #         if args.gt_file is None:
    #             if args.gt_only:
    #                 raise ValueError("gt_only模式需要提供--gt_file参数")
    ...
    #         # gt_only模式下不应该有pred_signal
    #         if args.gt_only and pred_signal is not None:
    #             print("  警告: gt_only模式下忽略pred_signal")
    #             pred_signal = None
            
    #         print(f"\n加载ground truth切片（条目索引: {args.entry_idx}）...")
    ...
    #             )
    #             if mod_from_file:
    #                 print(f"  从文件读取到调制方式: {mod_from_file}")
    #                 if args.modulation.upper() != mod_from_file.upper():
    #                     print(f"  警告: 指定的调制方式 ({args.modulation}) 与文件中的 ({mod_from_file}) 不一致")
    #             print(f"  GT信号长度: {len(gt_signal)} 采样点")
    #         except Exception as e:
    ...
    #     if args.gt_only:
    #         if pred_signal is not None:
    #             print("\n警告: gt_only模式下忽略pred_signal")
    #             pred_signal = None
    #         print("\n只测试ground truth模式（不加载预测信号）")
    ...
    #     auto_search = not args.no_auto_search
    #     # if args.loop_bandwidth is not None:
    #     #     print(f"使用指定的loop_bandwidth: {args.loop_bandwidth:.2e}")
    #     # elif auto_search:
    #     #     print("自动搜索最优loop_bandwidth...")
    #     # else:
    #     #     print("使用默认loop_bandwidth: 1e-7")

    #     snr_signal_1.append(batch_snr_db(pred_signal[0:1], gt_signal[0:1]))
    #     snr_signal_2.append(batch_snr_db(pred_signal[1:2], gt_signal[1:2]))
    #     result1 = test_single_pair(args,
    #         pred_signal[0], gt_signal[0], args.modulation,
    #         loop_bandwidth=args.loop_bandwidth,
    #         auto_search=auto_search
    #     )
    #     result2 = test_single_pair(args,
    #         pred_signal[1], gt_signal[1], args.modulation,
    #         loop_bandwidth=args.loop_bandwidth,
    #         auto_search=auto_search
    #     )
    ...
    #     # print("=" * 70)
    #     for k in range(2):
    #         result = result1 if k == 0 else result2
    #         result_file = output_dir / "demod_test_result.json"
    #         with open(result_file, 'w') as f:
    ...
    #         # print(f"  符号数: {result['gt_num_symbols']}")
    #         # print(f"  聚类得分: {result['gt_clustering_score']:.4f}")
    #         # print(f"  Loop bandwidth: {result['gt_info']['loop_bandwidth']:.2e}")
    #         # print(f"  Offset: {result['gt_info']['offset']}")
    #         if not args.disable_middle_output:
    #             if pred_signal is not None:
    #                 print(f"\n模型输出:")
    #                 print(f"  符号数: {result['pred_num_symbols']}")
    #                 print(f"  聚类得分: {result['pred_clustering_score']:.4f}")
    #                 print(f"  Loop bandwidth: {result['pred_info']['loop_bandwidth']:.2e}")
    #                 print(f"  Offset: {result['pred_info']['offset']}")
    #                 print(f"\n误符号率 (SER): {result['ser']:.6f}")
    #             else:
    #                 print("\n（未提供模型输出，仅测试ground truth）")
    #         # if k == 0:
    #     # if result1['gt_clustering_score'] >= 6:
    #     ber_signal_1.append(result1['ser'])
    #         # print('!!!!!!!!!!!!!!!!!!!!!!!!!')
    #         # if k == 1:
    #     # if result2['gt_clustering_score'] >= 6:
    #     ber_signal_2.append(result2['ser'])
    #     # if n >= 100:
    #     #     break
    data = (data1, data2, data3, data4)
    snr_signal_1, snr_signal_2, ber_signal_1, ber_signal_2, score_signal_1, score_signal_2 = run_all(
        data,
        args,
        output_dir="./results",
        num_workers=32
    ...
    avg_score_1 = np.nanmean(score_signal_1)
    avg_score_2 = np.nanmean(score_signal_2)
    ...
    print(f'\n平均聚类得分 signal 1: {avg_score_1:.6f}')
    print(f'平均聚类得分 signal 2: {avg_score_2:.6f}')
    print(f'平均聚类得分: {(avg_score_1 + avg_score_2)/2:.6f}')
