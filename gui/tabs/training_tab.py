...
class TrainingTab(QWidget):
    ...
    def init_ui(self):
        ...
        # 说明
        info_label = QLabel("训练扩散模型。训练数据路径会自动从配置文件中读取。")
        ...
        params_layout.addRow("训练轮数:", self.epochs_label)
        
        self.batch_size_label = QLabel("-")
        params_layout.addRow("批次大小:", self.batch_size_label)
        
        self.lr_label = QLabel("-")
        params_layout.addRow("学习率:", self.lr_label)
    ...
    def update_params_display(self):
        ...
        if not self.current_config_path:
            self.epochs_label.setText("-")
            self.batch_size_label.setText("-")
            self.lr_label.setText("-")
            return
        ...
        try:
            ...
            if 'training' in config:
                ...
                self.epochs_label.setText(str(epochs))
                ...
                if isinstance(lr, (int, float)):
                    # 处理科学计数法
                    if lr < 0.001:
                        self.lr_label.setText(f"{lr:.2e}")
                    else:
                        self.lr_label.setText(str(lr))
                else:
                    self.lr_label.setText(str(lr))
            else:
                self.epochs_label.setText("-")
            ...
            self.epochs_label.setText("读取失败")
            self.batch_size_label.setText("读取失败")
            self.lr_label.setText("读取失败")
    ...
    def start_training(self):
        ...
        command = [
            sys.executable,
            str(self.project_root / "train_PCMA_diffusion.py"),
            "--config",
            config_relative
        ]
    ...
    def on_finished(self, success, message):
        ...
        if success:
            ...
            self.status_label.setText(f"训练失败: {message}")
            QMessageBox.critical(self, "失败", f"模型训练失败: {message}")
